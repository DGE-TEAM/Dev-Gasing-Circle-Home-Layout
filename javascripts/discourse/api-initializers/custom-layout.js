// =============================================================
// GASING CIRCLE THEME — custom-layout.js
// Master-Detail layout for Gasing Academy News category
// Uses Discourse Plugin API v0.8+ (api-initializers)
// =============================================================
...
const SVG = {
  ...
  views: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>`,
  reply: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 14 4 9 9 4"/><path d="M20 20v-7a4 4 0 0 0-4-4H4"/></svg>`,
  ...
  chevLeft: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg>`,
  chevRight: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg>`,
...
function buildCalendarMonth(year, month, rangeStart, rangeEnd, isLeft) {
  ...
  // Day labels
  DAYS.forEach((d) => {
    html += `<div class="gc-cal-day-label">${d}</div>`;
  });

  // Empty cells
  for (let i = 0; i < firstDay; i++) {
    html += `<div class="gc-cal-day empty"></div>`;
  }
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    } else if (rangeStart && ts === rangeStart.getTime()) {
      cls += " selected";
    }
...
class GasingNewsLayout {
  ...
  async fetchTopics(sort = "latest") {
    ...
    try {
      const slug = TARGET_CATEGORY_SLUG;
      const order = sort === "trending" ? "?order=activity" : "";
      const res = await fetch(`/c/${slug}.json${order}`, {
        headers: { "X-Requested-With": "XMLHttpRequest" },
      });
      const data = await res.json();
      const raw = data?.topic_list?.topics || [];
      this.state.topics = raw.filter((t) => !t.pinned_globally || t.pinned);
    } catch (e) {
    ...
    this.container.innerHTML = `
      <div class="gc-category-layout">
        <div class="gc-hero-banner">
          <h1>Gasing Academy News ${SVG.newspaper}</h1>
  ...
  renderFeedLoading() {
    ...
    if (feed) {
      feed.innerHTML = `<div class="gc-loading"><div class="gc-spinner"></div> Memuat topik…</div>`;
    }
  }
  ...
  renderFeed() {
    ...
    if (topics.length === 0) {
      feed.innerHTML = `<div class="gc-empty-pane">${SVG.newspaper}<p>Tidak ada topik ditemukan</p></div>`;
      return;
    }
    ...
    feed.querySelectorAll(".gc-topic-card").forEach((card) => {
      card.addEventListener("click", () => {
        ...
        // Deselect all
        feed.querySelectorAll(".gc-topic-card").forEach((c) =>
    ...
    if (this.state.selectedTopic) {
      const sel = feed.querySelector(
        `[data-topic-id="${this.state.selectedTopic.id}"]`
      );
  ...
  _topicCardHTML(topic) {
    ...
    const imgHTML = imgUrl
      ...
      : `<div class="gc-card-image" style="height:100px;background:linear-gradient(135deg,var(--gc-blue-100),var(--gc-blue-50));display:flex;align-items:center;justify-content:center;">${SVG.newspaper}</div>`;

    const tag = (topic.tags || [])[0] || topic.category_name || "";
    const tagClass = getTagClass(tag);
    const tagHTML = tag
      ? `<span class="gc-tag-pill ${tagClass}" style="font-size:10px;padding:2px 7px;margin-bottom:4px;display:inline-flex;">${tag}</span>`
      : "";

    const excerpt = topic.excerpt
      ? topic.excerpt.replace(/<[^>]*>/g, "").slice(0, 100) + "…"
      : "";
    ...
    const avatarsHTML = posters
      .map((p) => {
        if (p.user?.avatar_template) {
          const avatarUrl = p.user.avatar_template.replace("{size}", "44");
          return `<img src="${avatarUrl}" class="" style="width:22px;height:22px;border-radius:50%;border:2px solid white;margin-left:-6px;object-fit:cover;" alt="${p.user.name || ""}" />`;
        }
        return `<div class="gc-avatar-placeholder" style="width:22px;height:22px;border-radius:50%;border:2px solid white;margin-left:-6px;background:var(--gc-blue-400);display:inline-flex;align-items:center;justify-content:center;font-size:9px;font-weight:700;color:white;">${getInitials(p.user?.name || "?")}</div>`;
      })
      .join("");
        ...
        <div class="gc-card-body">
          ...
          <div class="gc-card-footer">
            ...
            <div style="display:flex;align-items:center;gap:8px;">
              <div class="gc-card-authors" style="display:flex;">${avatarsHTML}</div>
  ...
  renderReadingPane() {
    ...
    if (!topic || posts.length === 0) {
      pane.innerHTML = `<div class="gc-pane-inner"><div class="gc-empty-pane">${SVG.newspaper}<p>Pilih artikel untuk mulai membaca</p></div></div>`;
      return;
    }
    ...
    // Extract first image from post if exists
      ...
      <div class="gc-pane-inner">
        ...
        ${postImage ? `
          <figure class="gc-pane-featured-image">
            ...
            ${firstPost?.username ? `<figcaption>Dokumentasi kegiatan Gasing Academy</figcaption>` : ""}
          </figure>` : ""}
  ...
  _sanitizePostBody(cooked, excludeImgSrc) {
    // Remove first image if already shown as featured
    if (excludeImgSrc) {
    ...
    // Remove lightbox wrappers but keep content
  ...
  _repliesHTML(posts) {
    ...
    posts.slice(0, 6).forEach((post, i) => {
      ...
      const avatarUrl = post.avatar_template
        ? post.avatar_template.replace("{size}", "72")
        : null;
      ...
      const isNested = i > 0 && i % 3 === 2; // demo nesting pattern

      // Role badge logic (demo)
      const roleName = i === 0 ? "Trainer Utama" : i === 1 ? "Trainer Kelas" : "Trainer Utama";

      const excerpt = post.cooked
        ? post.cooked.replace(/<[^>]*>/g, "").slice(0, 100)
        : "(no content)";
  ...
  renderFilterPopup() {
    ...
    if (!popup) return;
    popup.innerHTML = FILTER_LABELS.map(
      (label) => `
    ...
    popup.querySelectorAll(".gc-filter-item").forEach((item) => {
      item.addEventListener("click", (e) => {
        ...
        if (this.state.activeFilters.has(label)) {
          this.state.activeFilters.delete(label);
          cb.checked = false;
        } else {
          this.state.activeFilters.add(label);
          cb.checked = true;
        }
        this.renderFeed();
      });
    ...
    popup.querySelectorAll(".gc-cal-day:not(.empty):not(.disabled)").forEach((day) => {
      day.addEventListener("click", (e) => {
        ...
        if (!this.state.rangeStart || (this.state.rangeStart && this.state.rangeEnd)) {
          this.state.rangeStart = d;
          this.state.rangeEnd = null;
        } else {
          if (d >= this.state.rangeStart) {
            this.state.rangeEnd = d;
          } else {
  ...
  _bindPaneEvents(pane) {
    ...
    pane.querySelectorAll("[data-post-id]").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        // Close others
        pane.querySelectorAll(".gc-more-dropdown").forEach((d) => {
...
export default apiInitializer("0.8", (api) => {
  // We attach to the route did-transition hook so we can
  // inject our custom layout into the main outlet DOM node
  // every time the user navigates to the target category.
  ...
  function mountLayout() {
    ...
    // Find a stable anchor in the Discourse outlet
    const outlet =
  ...
  // Hook into Discourse's router events
  ...
  if (typeof document !== "undefined") {
    if (document.readyState === "complete") {
      setTimeout(mountLayout, 150);
    } else {
      window.addEventListener("load", () => setTimeout(mountLayout, 150));
    }
  }

  // MutationObserver fallback: watch for the main outlet to be populated
  // (Discourse sometimes renders asynchronously via Ember)
